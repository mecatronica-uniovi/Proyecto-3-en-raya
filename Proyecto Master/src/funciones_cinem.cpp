...
extern Fdt FuncionTrans;
extern RegPID Regulador;
float Ts = 0.05;
...
void desplazamientoRobot(array<double, 4> ang){
    array<double, 4> ang_motor;
    array<double, 4> PWM;
    int motores=0;
    
    ang_motor[0]=relacionLineal(ang[0], 90, -90, 180, 0);
    ang_motor[1]=relacionLineal(ang[1], 90, -90, 180, 0);
    ang_motor[2]=relacionLineal(ang[2], 150, -30, 180, 0);
    ang_motor[3]=relacionLineal(ang[3], 135, -45, 180, 0);

    for(int i=0;i<4;i++){
        PWM[i]=relacionLineal(ang_motor[i], 180, 0, PWM_MAX, PWM_MIN);
        motores=motores+1;
    }
    //SetPWM(motores, PWM);
}
...
void jugada(array<double, 3> xyz_0, array<double, 3> xyz_f, array<double, 4> L){
    array<double,3> pos_inicial={xyz_0[0], xyz_0[1], xyz_0[2]+100};
    array<double,3> pos_final={xyz_f[0], xyz_f[1], xyz_f[2]+100};

    /*Colocarse encima de pos inicial*/
    movimiento_segun_coord(pos_inicial, L);
    // while (!Final_Carrera()){
    //     continue;
    // }

    // /*Bajar a coger pieza en pos inicial*/
    // movimiento_segun_coord(xyz_0, L);
    // while (!MotorReady){
    //     continue;
    // }
    // muevePinza();
    ...
    // /*Sube encima de la pos inicial con la pieza*/
    // movimiento_segun_coord(pos_inicial, L);
    // while (!MotorReady){
    //     continue;
    // }

    // /*Avanza a la pos final pero encima de ella*/
    // movimiento_segun_coord(pos_final, L);
    // while (!MotorReady){
    //     continue;
    // }

    // /*Baja a la pos final y abre la pinza*/
    // movimiento_segun_coord(xyz_f, L);
    // while (!MotorReady){
    //     continue;
    // }
    // muevePinza();
...
void ejecutarSimulacion(){
    const int n = 100;
    float Ref = 1, Uk = 0, error = 0, Yk = 0;
    
    cout<<"Iniciando simulacion..."<<endl;;
    
    for (int i = 0; i < n; i++) {
        error = Ref - Yk;
        Uk = Regulador.calcularUk(error);
        
        cout<<"U["<<i<<"]= "<<Uk;

        Yk = FuncionTrans.CalcularSalida(Uk);
        ...
        if (Serial.available() > 0) {
            ...
            if (cmd.startsWith("STOP")) {
                Serial.println("Simulacion interrumpida");
                return;
            }
        }
    }
    Serial.println("Simulacion completada");
}